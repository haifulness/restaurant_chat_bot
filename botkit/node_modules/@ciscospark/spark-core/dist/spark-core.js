...
var SparkCore = _ampersandState2.default.extend((_obj = {
  ...
  initialize: function initialize() {
    ...
    this.config = (0, _merge3.default)({}, _config2.default, this.config);
  ...
  logout: function logout() {
    ...
    return _promise2.default.resolve().then(function () {
      if (_this4.mercury) {
        return _this4.mercury.disconnect();
      }
      return _promise2.default.resolve();
    }).then(function () {
      if (_this4.device) {
        return _this4.device.unregister().catch(function () {
          return _promise2.default.resolve();
        });
      }
      return _promise2.default.resolve();
    }).then(function () {
      return _promise2.default.all([_this4.boundedStorage.clear(), _this4.unboundedStorage.clear()]);
    }).then(function () {
      var _credentials4;

      return (_credentials4 = _this4.credentials).logout.apply(_credentials4, args);
    });
  },
  ...
  upload: function upload(options) {
    ...
    var promise = this._uploadPhaseInitialize(options).then(function () {
      var p = _this5._uploadPhaseUpload(options);
      ...
      return _this5._uploadPhaseFinalize.apply(_this5, [options].concat(args));
    }).then(function (res) {
      return res.body;
    });
  ...
  _uploadPhaseInitialize: function _uploadPhaseInitialize(options) {
    ...
    return this.request(options.phases.initialize).then(function () {
      ...
      return _this6._uploadApplySession.apply(_this6, [options].concat(args));
    }).then(function (res) {
      _this6.logger.debug('client: initiated upload session');
      return res;
    });
  },
  ...
  _uploadPhaseUpload: function _uploadPhaseUpload(options) {
    ...
    var promise = this.request(options.phases.upload).then(function (res) {
      _this7.logger.debug('client: uploaded file');
      return res;
    });
    ...
    if (process.env.NODE_ENV === 'test') {
      promise.on('progress', function (event) {
        _this7.logger.info('upload progress', event.loaded, event.total);
      });
    }
  ...
  _uploadPhaseFinalize: function _uploadPhaseFinalize(options) {
    ...
    return this.request(options.phases.finalize).then(function (res) {
      _this8.logger.debug('client: finalized upload session');
      return res;
    });
  }
...
function registerPlugin(name, constructor, options) {
  ...
  if (!children[name] || options.replace) {
    ...
    if (options.config) {
      (0, _merge3.default)(_config2.default, options.config);
    }

    if ((0, _has3.default)(options, 'payloadTransformer.predicates')) {
      _config2.default.payloadTransformer.predicates = _config2.default.payloadTransformer.predicates.concat((0, _get3.default)(options, 'payloadTransformer.predicates'));
    }

    if ((0, _has3.default)(options, 'payloadTransformer.transforms')) {
      _config2.default.payloadTransformer.transforms = _config2.default.payloadTransformer.transforms.concat((0, _get3.default)(options, 'payloadTransformer.transforms'));
    }
